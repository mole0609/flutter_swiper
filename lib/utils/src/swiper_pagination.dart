...
import 'package:swiper_demo/utils/flutter_page_indicator.dart';
...
class MyIndSwiperPaginationBuilder extends SwiperPlugin {
  ///color when current index,if set null , will be Theme.of(context).primaryColor
  final Color activeColor;
  ...
  const MyIndSwiperPaginationBuilder(
      {this.activeColor,
        this.color,
        this.key,
        this.size: 10.0,
  ...
  Widget build(BuildContext context, SwiperPluginConfig config) {
    ...
    if (config.indicatorLayout != PageIndicatorLayout.MYIND &&
        config.layout == SwiperLayout.DEFAULT) {
      return new PageIndicator(
        count: config.itemCount,
        controller: config.pageController,
